<?php function roots_comments($comment, $args, $depth) {
	...
	<li <?php comment_class(); ?>>
		<article id="comment-<?php comment_ID(); ?>">
			<div class="gravatar span-3">
				<?php echo get_avatar($comment,$size='64'); ?>
			</div>
			<div class="comment-details span-19 last">
				<header class="comment-author vcard">
					<?php printf(__('<cite class="fn">%s</cite>', 'roots'), get_comment_author_link()) ?> on 
					<time datetime="<?php echo comment_date('c') ?>"><a href="<?php echo htmlspecialchars( get_comment_link( $comment->comment_ID ) ) ?>"><?php printf(__('%1$s', 'roots'), get_comment_date('l, j F Y - G:i'),  get_comment_time()) ?></a></time>
				...
				<?php $comment_string = get_comment_reply_link(array_merge( $args, array('depth' => $depth, 'max_depth' => $args['max_depth'])));

					$comment_string = str_replace ( 'class=\'' , 'class=\'button ', $comment_string);
					echo $comment_string;
				 ?>
...
<section id="respond">
	...
	<form action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" method="post" id="commentform">
		...
		<p><input name="submit" class="button left" type="submit" id="submit" tabindex="5" value="<?php _e('Submit Comment', 'roots'); ?>"></p>
